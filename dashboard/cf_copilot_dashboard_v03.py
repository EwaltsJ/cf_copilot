...
# ═══════════════════════════════════════
# MOCK DATA (isolated — replace with real API calls)
# ═══════════════════════════════════════
...
if st.session_state.weekly_forecast is None:
    ...
    if st.session_state.predictions_df is not None:
        ...
        # Top 10 riskiest — split layout: table left, detail panel right
        top10 = (
        ...
        with col_table:
            ...
            # Display table with risk colour badges
            display_rows = []
        ...
        with col_panel:
            if selected_opt != "Select an invoice…":
                ...
                <div class="invoice-panel" style="margin-top:2.6rem;">
                    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1.2rem;">
                        <div>
                            ...
                            <div style="font-size:0.85rem;color:#6b7fa3;margin-top:0.2rem;">
                                {invoice.get('name_customer','Unknown')}
                            </div>
                        </div>
                    ...
                    <div style="font-family:'DM Mono',monospace;font-size:0.88rem;color:#c9d4e8;margin-bottom:1.4rem;">
                        {invoice.get('due_in_date','—')}
                    </div>
                </div>
                """, unsafe_allow_html=True)
